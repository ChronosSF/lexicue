...
describe("FileTable", () => {
  it("shows what the app understood and the price on both lanes", () => {
    ...
    render(<FileTable files={files} lane="fast" onRemove={() => undefined} />);
  ...
  it("explains an unusable file in place and still lets it be removed", async () => {
    ...
    render(<FileTable files={files} lane="economy" onRemove={onRemove} />);
  ...
  it("marks the lane the user chose", () => {
    ...
    const { container } = render(
      <FileTable files={files} lane="economy" onRemove={() => undefined} />,
    );
